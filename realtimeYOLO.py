...
def load_yolo(args):
    net = cv2.dnn.readNet(f'./yolov7-tiny.weights', f'./yolov7-tiny.cfg')
    classes = []
...
def main():
    ...
    # Initialise detection counters
    detection_counter = 0
    lagged_detection_counter = 0

    while not quit_keypress():
        previous_detection_counter = detection_counter
        detection_counter = 0
        if aria.CameraId.Rgb in observer.images:
            ...
            # Increment counter for 'person' and 'car' detections
            for class_id in class_ids:
                if classes[class_id] in ['person', 'car', 'bicycle', 'motorbike', 'bus', 'truck']:
                    detection_counter += 1

            cv2.imshow(rgb_window, img_with_boxes)
            del observer.images[aria.CameraId.Rgb]
            print(f"Live # of detections: {detection_counter}")

        # Update lagged_detection_counter
        if detection_counter >= lagged_detection_counter:
            lagged_detection_counter = detection_counter
        elif detection_counter < lagged_detection_counter:
            decrement = (lagged_detection_counter - detection_counter) / (2 * 10)  # Lags by 2 seconds. RGB camera streams at 10 FPS 
            lagged_detection_counter = max(detection_counter, lagged_detection_counter - decrement)

        print(f"Traffic score: {lagged_detection_counter}")
